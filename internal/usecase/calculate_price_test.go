...
import (
	...
	"github.com/carlos-henrique-dev/gointensivo/internal/infra/database"
	"github.com/carlos-henrique-dev/gointensivo/internal/order/entity"
...
func (suite *CalculatePriceUseCaseTestSuite) TestCalculateFinalPrice() {
	...
	calculateFinalPriceUserCase := NewCalculateFinalPriceUseCase(suite.OrderRepository)
	output, err := calculateFinalPriceUserCase.Execute((calculateFinalPriceInput))
	suite.NoError(err)
